    ...
    <div className="md:hidden border-t border-gray-100 bg-white max-h-[80vh] overflow-y-auto">
      ...
      <div className="p-4 border-b border-gray-50">
        <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
          ...
          <input placeholder="Search destinations, tours…" className="flex-1 text-sm bg-transparent focus:outline-none placeholder:text-gray-400" />
        </div>
      ...
      <div className="py-2">
        {navItems.map(item => {
          ...
          if (!hasMenu) return (
            <Link key={item.key} href={'href' in item ? item.href : '#'} onClick={onClose}
                      ...
                      <ul className="space-y-1">
                        {section.items.map(menuItem => (
                          <li key={menuItem.label}>
                            <Link href={menuItem.href} onClick={onClose}
                              className="flex items-center justify-between py-2 text-sm text-gray-700 hover:text-green-600 transition-colors">
                              <span>{menuItem.label}</span>
                              {'description' in menuItem && <span className="text-xs text-gray-400 hidden xs:block">{menuItem.description}</span>}
                            </Link>
                          </li>
                        ))}
                      </ul>
        ...
        {session ? (
          ...
          <div className="flex flex-col gap-2">
            ...
            <AuthModal defaultTab="register" trigger={
              <Button className="w-full bg-green-500 hover:bg-green-600 text-white rounded-xl py-2.5 text-sm font-medium flex items-center justify-center gap-2">
