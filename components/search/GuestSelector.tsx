      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-gray-100 shadow-xl z-50 p-4">
          <div className="divide-y divide-gray-50">
            <Counter label="Adults" sub="Ages 13+" value={value.adults} onChange={v => onChange({ ...value, adults: v })} min={1} max={20} />
            <Counter label="Children" sub="Ages 2–12" value={value.children} onChange={v => onChange({ ...value, children: v })} min={0} max={10} />
          </div>
          <button onClick={() => setOpen(false)} className="w-full mt-3 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-700 transition-colors">
            Done
          </button>
        </div>
      )}
