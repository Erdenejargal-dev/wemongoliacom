...
import type { GuideReview } from '@/lib/api/guides'
...
export function GuideReviews({ reviews, rating, reviewsCount }: GuideReviewsProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h2 className="text-base font-bold text-gray-900 mb-1">{g.reviewsTitle}</h2>
          <p className="text-xs text-gray-500">{g.reviewsVerified(reviewsCount)}</p>
        </div>
        <div className="text-center">
          <p className="text-3xl font-bold text-gray-900">{rating.toFixed(1)}</p>
          <StarRow rating={Math.round(rating)} />
          <p className="text-xs text-gray-500 mt-0.5">{g.reviewsOverall}</p>
        </div>
      </div>
      ...
      <div className="space-y-5">
        {reviews.map(review => (
          <div key={review.id} className="relative border border-gray-100 rounded-2xl p-5 hover:border-brand-100 hover:bg-brand-50/30 transition-colors">
            ...
            {review.tourName && (
              <p className="text-[10px] text-gray-400 mt-2 font-medium">{g.reviewTourLine(review.tourName)}</p>
            )}
          </div>
        ))}
      </div>
