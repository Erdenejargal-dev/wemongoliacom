...
export default function DashboardOverviewPage() {
  return (
    <div className="space-y-6">
      <PageHeader title="Overview" description="Welcome back! Here's what's happening with your business." />

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Bookings" value={summaryStats.totalBookings} icon={BookOpen} trend={12} trendLabel="vs last month" iconColor="text-blue-600" iconBg="bg-blue-50" />
        <StatCard title="Monthly Revenue" value={`$${(summaryStats.monthlyRevenue / 1000).toFixed(1)}k`} icon={DollarSign} trend={8} trendLabel="vs last month" iconColor="text-green-600" iconBg="bg-green-50" />
        <StatCard title="Upcoming Tours" value={summaryStats.upcomingTours} icon={CalendarCheck} trend={-3} trendLabel="vs last month" iconColor="text-purple-600" iconBg="bg-purple-50" />
        <StatCard title="Avg. Rating" value={summaryStats.averageRating} icon={Star} trend={2} trendLabel="vs last month" iconColor="text-amber-600" iconBg="bg-amber-50" />
      </div>
