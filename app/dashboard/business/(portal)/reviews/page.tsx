...
function ReviewCard({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const updated = await replyToProviderReview(review.id, replyText.trim(), freshToken)
      if (updated) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 sm:p-5 shadow-sm">
      ...
      {!review.providerReply && !showReplyForm && (
        <button
          type="button"
      ...
      {showReplyForm && (
        <form onSubmit={handleSubmit} className="mt-4">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button
              type="button"
              onClick={() => { setShowReplyForm(false); setReplyText(''); setError(null) }}
...
export default function ReviewsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await fetchProviderReviews(freshToken, { limit: 50 })
  ...
  if (!token) {
    return (
      <div className="space-y-4">
        <PageHeader title="Reviews" description="What travelers are saying about your services" />
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Reviews"
        description={loading ? 'Loading…' : `What travelers are saying — ${total} review${total !== 1 ? 's' : ''}`}
        actions={
          <button
            onClick={load}
            disabled={loading}
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map(r => (
            <ReviewCard key={r.id} review={r} onReplySuccess={load} />
          ))}
        </div>
      )}
    </div>
