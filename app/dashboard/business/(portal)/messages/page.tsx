...
import { MessageSquare, ArrowLeft, Loader2, Send, RefreshCw } from 'lucide-react'
    ...
    <div className={`flex items-end gap-2 ${isProvider ? 'flex-row-reverse' : 'flex-row'}`}>
      <div className={`max-w-[85%] sm:max-w-[75%] ${isProvider ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
          isProvider ? 'bg-brand-600 text-white rounded-br-sm' : 'bg-white border border-gray-200 text-gray-800 rounded-bl-sm shadow-sm'
        }`}>{msg.text}</div>
...
export default function MessagesPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      else setError(e instanceof Error ? e.message : mt.errorLoading)
  ...
  const loadMessages = useCallback(async (convId: string) => {
    const ft = token ? await getFreshAccessToken() : null
    if (!ft) return
    setMessagesLoading(true); setSendError(null)
    try {
      ...
      if (e instanceof ApiError && e.status === 401) { await signOut({ redirect: false }); router.push('/auth/login') }
      else setSendError(e instanceof Error ? e.message : mt.errorLoading)
  ...
  return (
    <div className="space-y-0 -mx-4 sm:-mx-6 lg:-mx-0">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden" style={{ minHeight: 'calc(100vh - 220px)', maxHeight: '600px' }}>
        <div className="flex h-full" style={{ minHeight: '500px' }}>
          ...
          <div className={`w-full lg:w-80 shrink-0 border-r border-gray-100 flex flex-col overflow-hidden ${mobileView === 'chat' ? 'hidden lg:flex' : 'flex'}`}>
            <div className="p-3 border-b border-gray-100 shrink-0">
              <p className="text-xs text-gray-500">{mt.conversations(conversations.length)}</p>
            </div>
                  ...
                  <button key={conv.id} onClick={() => handleSelect(conv.id)}
                    ...
                    <div className="relative shrink-0">
                      <div className="w-11 h-11 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                        {conv.traveler.avatarUrl ? <img src={conv.traveler.avatarUrl} alt={name} className="w-full h-full object-cover" />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <p className={`text-sm truncate ${activeId === conv.id ? 'font-semibold text-brand-800' : 'font-medium text-gray-900'}`}>{name}</p>
                        <span className="text-[10px] text-gray-400 shrink-0">{formatTime(conv.lastMessageAt, t.dateLocale, mt.yesterday)}</span>
                      </div>
                      {conv.listingType && <p className="text-[10px] text-gray-400 capitalize mb-0.5">{mt.inquiry(conv.listingType)}</p>}
          ...
          <div className={`flex-1 flex flex-col min-w-0 ${mobileView === 'list' ? 'hidden lg:flex' : 'flex'}`}>
            {active ? (
              <>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-white shrink-0">
                  ...
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center shrink-0">
                    {active.traveler.avatarUrl ? <img src={active.traveler.avatarUrl} alt={travelerName} className="w-full h-full object-cover" />
                      : <span className="text-gray-500 font-semibold">{travelerName.charAt(0) || '?'}</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{travelerName}</p>
                    {active.listingType && <p className="text-xs text-gray-500 capitalize">{mt.inquiry(active.listingType)}</p>}
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-gray-50/50">
                  {messagesLoading ? (
                    ...
                    <>
                      {messages.map(msg => <MessageBubble key={msg.id} msg={msg} isProvider={msg.senderRole === 'provider'} />)}
                ...
                <div className="border-t border-gray-100 bg-white px-4 py-3 shrink-0">
                  ...
                  <div className="flex items-end gap-2">
                    <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                      placeholder={mt.typeReply} rows={1} maxLength={2000}
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if (replyText.trim()) handleSend(replyText.trim()) } }}
                    ...
                    <button type="button" onClick={() => { if (replyText.trim()) handleSend(replyText.trim()) }}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-sm font-medium text-gray-500">{mt.selectConversation}</p>
              </div>
            )}
          </div>
        </div>
