...
export default function TripsPage() {
  ...
  async function loadTrips() {
    ...
    if (!ft) { setTrips([]); setLoading(false); setError('Not signed in.'); return }
    setLoading(true); setError(null)
    try {
      const bookings = await fetchMyBookings(ft)
      const mapped = bookings.map(mapBackendBookingToTripCard).filter((ti): ti is Trip => Boolean(ti))
  ...
  async function handleCancel(bookingCode: string) {
    ...
    if (!ft) { setError('Session expired.'); await signOut({ redirect: false }); router.push('/auth/login'); return }
    const reason = window.prompt('Reason for cancellation (optional):')
    try {
      await cancelBooking(bookingCode, reason ?? '', ft)
      await loadTrips()
    } catch (e: unknown) {
      if (e instanceof ApiError && e.status === 401) { await signOut({ redirect: false }); router.push('/auth/login') }
      else setError(e instanceof Error ? e.message : 'Failed to cancel booking.')
    }
  }

  const sections: { label: string; status: TripStatus; accent: string }[] = [
    { label: tt.upcoming,  status: 'Upcoming',  accent: 'text-brand-600 bg-brand-50 border-brand-100' },
    { label: tt.completed, status: 'Completed', accent: 'text-blue-600 bg-blue-50 border-blue-100'   },
    { label: tt.cancelled, status: 'Cancelled', accent: 'text-red-500 bg-red-50 border-red-100'      },
  ]

  const grouped = useMemo(() => ({
    Upcoming:  trips.filter(tr => tr.status === 'Upcoming'),
    Completed: trips.filter(tr => tr.status === 'Completed'),
    Cancelled: trips.filter(tr => tr.status === 'Cancelled'),
  }), [trips])
  ...
  return (
    <div className="min-h-screen bg-gray-50/40">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-bold text-gray-900">{tt.pageTitle}</h1>
          </div>
          <p className="text-xs text-gray-500">{tt.pageSubtitle}</p>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        {loading && (
          <div className="text-center py-16 text-sm text-gray-500">{tt.loading}</div>
        )}
        {!loading && error && (
          <div className="text-center py-16">
            <p className="text-sm font-semibold text-red-600">{error}</p>
            <p className="text-xs text-gray-500 mt-1">{tt.errorRetry}</p>
          </div>
        )}
        {!loading && !error && trips.length === 0 && (
          <div className="text-center py-20">
            <Compass className="w-12 h-12 text-gray-300 mx-auto mb-3" />
            <p className="text-gray-500 font-semibold text-base mb-1">{tt.noTrips}</p>
            <p className="text-gray-400 text-sm mb-5">{tt.noTripsDesc}</p>
            <Link href="/tours"
              ...
              <Compass className="w-4 h-4" />{tt.exploreBtn}
            </Link>
          </div>
        )}
          ...
          <section key={s.status}>
            ...
            {loading ? null : grouped[s.status].length === 0 ? (
              <div className="text-center py-10 bg-white rounded-2xl border border-gray-100 shadow-sm">
                ...
                <p className="text-gray-500 font-medium text-sm">{tt.noUpcoming}</p>
                {s.status === 'Upcoming' && (
                  <>
                    <p className="text-gray-400 text-xs mt-1 mb-3">{tt.noUpcomingDesc}</p>
                    <Link href="/tours"
                      ...
                      <Compass className="w-3.5 h-3.5" />{tt.exploreBtn}
                    </Link>
                  </>
                )}
              </div>
