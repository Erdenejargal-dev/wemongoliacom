    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-gray-50 py-12 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-green-100 mb-4 animate-[ping_0.4s_ease-out_forwards]">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-md overflow-hidden mb-5">
          ...
          <div className="p-5 space-y-3">
            <Detail icon={<MapPin className="w-4 h-4 text-green-500" />} label="Location" value={booking.tourLocation} />
            <Detail icon={<CalendarDays className="w-4 h-4 text-green-500" />} label="Travel Date" value={formatDate(booking.date)} />
            <Detail icon={<Users className="w-4 h-4 text-green-500" />} label="Guests" value={`${booking.guests} guest${booking.guests !== 1 ? 's' : ''}`} />
            <Detail icon={<Clock className="w-4 h-4 text-green-500" />} label="Duration" value={booking.tourDuration} />
          </div>
        </div>
        ...
        <div className="space-y-3">
          <Link href="/tours"
            className="w-full py-3.5 bg-green-500 hover:bg-green-600 text-white font-bold text-sm rounded-2xl transition-colors flex items-center justify-center gap-2">
            Browse More Tours
